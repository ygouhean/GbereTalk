...
// Se connecter avec les informations utilisateur 
...
// Fonction pour initialiser les données utilisateur depuis localStorage
function initializeUserDataFromStorage() {
  if (record && record.data && record.data.user) {
    const userInfo = record.data.user;
    
    // Mise à jour des éléments avec les données du localStorage
    const userName = document.querySelectorAll(".user_name");
    const userEmail = document.querySelectorAll(".user_email");
    const userLocation = document.querySelectorAll(".user_location");
    const preview = document.querySelectorAll(".user-profile-image");
    
    // Mise à jour des noms
    Array.from(userName).forEach((element) => {
      element.innerHTML = userInfo.name || "Utilisateur";
    });
    
    // Mise à jour des emails
    Array.from(userEmail).forEach((element) => {
      element.innerHTML = userInfo.email || "email@exemple.com";
    });
    
    // Mise à jour des localisations
    Array.from(userLocation).forEach((element) => {
      element.innerHTML = userInfo.location || "Non spécifiée";
    });
    ...
    // Mise à jour de la description (toujours remplacer la valeur par défaut)
    ...
    if (profileDescription) {
      if (userInfo.description) {
        ...
        console.log("Description chargée depuis localStorage:", userInfo.description);
      } else {
        // Si pas de description personnalisée, garder la description par défaut
        console.log("Aucune description personnalisée trouvée, utilisation de la description par défaut");
      }
    }
    ...
    if (statusDisplay) {
      const status = userInfo.status || 'Disponible';
      statusDisplay.innerHTML = `${status} <i class="mdi mdi-chevron-down"></i>`;
      console.log("Statut chargé depuis localStorage:", status);
    }
    
    // Mise à jour des paramètres de notification depuis localStorage
    const notificationSwitch = document.getElementById("security-notificationswitch");
    const mutedSwitch = document.getElementById("notification_muted_switch");
    
    if (notificationSwitch && userInfo.notification !== undefined) {
      notificationSwitch.checked = userInfo.notification == 1;
      console.log("Switch notification initialisé depuis localStorage:", notificationSwitch.checked);
    }
    if (mutedSwitch && userInfo.is_muted !== undefined) {
      mutedSwitch.checked = userInfo.is_muted == 1;
      console.log("Switch muted initialisé depuis localStorage:", mutedSwitch.checked);
    }
    
    console.log("Données utilisateur initialisées depuis localStorage");
  }
}

// Fonction pour forcer la récupération de la description depuis le serveur
function forceUpdateUserDescription() {
  if (userId) {
    console.log("Force la récupération de la description utilisateur depuis le serveur");
    socket.emit("currentUser", { userId });
  }
}

// Initialiser les données au chargement avec un délai pour s'assurer que le DOM est prêt
document.addEventListener('DOMContentLoaded', function() {
  setTimeout(() => {
    initializeUserDataFromStorage();
    // Forcer la récupération des données depuis le serveur après l'initialisation
    setTimeout(() => {
      forceUpdateUserDescription();
    }, 1000);
  }, 500);
});

// Aussi initialiser immédiatement au cas où le DOM serait déjà chargé
...
function fileValidation() {
  ...
  if (!allowedExtensions.exec(filePath)) {
    toastr.error(`Veuillez sélectionner uniquement un fichier audio, vidéo ou image valide.`, "Erreur");
    fileInput.value = '';
  }
  else {
    // Image preview
    if (fileInput.files && fileInput.files[0]) {
      if (fileSize > 2097152) {
        toastr.error(`Veuillez sélectionner un fichier inférieur à 2 Mo`, "Erreur");
        fileInput.value = '';
      }
    }
  }
...
contactForm.addEventListener("submit", (e) => {
  ...
  if (name == '') {
    toastr.error('Le nom du contact est requis !', 'Erreur');
  }
  created_by = userId;
          ...
          <div class="d-flex align-items-center" onclick="singleChat('${contact.user_id}')">
            ...
            <div class="dropdown">
              ...
              <div class="dropdown-menu dropdown-menu-end">
                <a class="dropdown-item delete-contact" href="javascript:void(0);" onclick="deleteContact('${contact._id}', '${contact.user_id}')">Supprimer <i
...
messageForm.addEventListener("submit", (e) => {
  ...
  if (messageId) {
    var editMessage = flag == '1' ? "(modifié)":flag == '3'? "(Transféré)":"";
...
socket.on("chat message", function ({ id, message, sender_id, receiver_id, file_upload, createdAt, receiverName, receiverImage, myid, flag}) {
  ...
  if (flag == '3') {
    setTimeout(() => {
      document.querySelector(".msg_" + id + " .edit-flag").innerHTML = "(Transféré)";
    }, 500);
  }
            ...
            <div class="user-chat-content">
              <div class="ctext-wrap">
                ...
                <div class="dropdown align-self-start">
                  ...
                  <div class="dropdown-menu dropdown-menu-end">
                    <a class="dropdown-item" href="javascript:void(0);" id="${id}" onclick="singleMessageCopy(this)" style="display:${none_editBtn}">Copier <i class="ri-file-copy-line float-end text-muted"></i></a>
                    <a class="dropdown-item" href="javascript:void(0);" onclick="singleForwordMessage(this)" id="${id}" type="single" data-bs-toggle="modal" data-bs-target="#forwardContact-Modal">Transférer<i class="ri-chat-forward-line float-end text-muted"></i></a>
                  </div>
                </div>
              </div>
            ...
            <div class="user-chat-content">
              <div class="ctext-wrap">
                ...
                <div class="dropdown align-self-start">
                  ...
                  <div class="dropdown-menu dropdown-menu-end">
                    <a class="dropdown-item" href="javascript:void(0);" id="${id}" onclick="singleMessageCopy(this)" style="display:${none_editBtn}">Copier <i class="ri-file-copy-line float-end text-muted"></i></a>
                        <a class="dropdown-item" href="javascript:void(0);" id="${id}" onclick="singleMessageUpdate(this)" style="display:${none_editBtn}">Modifier <i class="ri-save-line float-end text-muted"></i></a>
                        <a class="dropdown-item" href="javascript:void(0);" onclick="singleForwordMessage(this)" id="${id}" type="single" data-bs-toggle="modal" data-bs-target="#forwardContact-Modal">Transférer <i class="ri-chat-forward-line float-end text-muted"></i></a>
                    <a class="dropdown-item" href="javascript:void(0);" id="${id}" onclick="singleMessageDelete(this)">Supprimer <i
                        class="ri-delete-bin-line float-end text-muted"></i></a>
                  </div>
                </div>
              </div>
          ...
          <div class="ms-4 me-0">
            <ul class="list-inline mb-0 font-size-18">
              ...
              <li class="list-inline-item dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-end">
                  <a class="dropdown-item" href="javascript:void(0);" id="${id}" onclick="singleMessageDelete(this)">Supprimer</a>
                </div>
              </li>
            </ul>
          </div>
...
// Charger les informations de l'utilisateur actuel au démarrage
socket.emit("currentUser", { userId });
...
socket.on('contactInfo', ({ contacts }) => {
  ...
  `;
    ...
    const receiverData = `
        <div class="float-end">
          <button type="button" class="btn btn-light btn-sm" id="receiver_edit" onclick="edit_receiverName(this)"><i class="ri-edit-fill me-1 ms-0 align-middle"></i> Modifier</button>
        </div>
        <div>
...
socket.on('onlineUser', ({ online }) => {
  setTimeout(function () {
    ...
    var receiver_status = online == 1 ? `<i class="ri-record-circle-fill font-size-10 text-success d-inline-block ms-1"></i> Connecté(e)` : '<i class="ri-record-circle-fill font-size-10 text-secondary d-inline-block ms-1"></i> Déconnecté(e)';
...
function CharacterData(id, name) {
  ...
  socket.emit("userClick", { id, userId, receiverId, startm: 0 });
   ...
   socket.on("userMessage", ({ users, msgno }) => {
    ...
    // Charger tous les messages reçus
    adduchat(users, receiverName);
    
    // Mettre à jour le compteur pour la pagination
    startm = users.length;
...
function addLoadMoreButton() {
  const loadMoreBtn = `
    <div class="text-center p-3">
      <button class="btn btn-link" onclick="loadMoreMessages()">
        Charger plus de messages
      </button>
    </div>
  `;
  $('.messages__history').prepend(loadMoreBtn);
}
...
let adduchat = (users, receiverName) => {
  ...
  users.forEach((user, index) => {
    ...
    const createdAt =
      ...
      ":" +
      time.getMinutes();
      var editMessage = user.flag == '1' ? "(modifié)":user.flag == '3'? "(Transféré)":"";
    ...
    if (imgList.includes(extName)) {
      ...
      var message = `<p class="mb-0 single_message text-break">${user.flag == '2' ? "Message supprimé":user.message}</p>`;
    }
              ...
              <div class="user-chat-content">
                ...
                <div class="ctext-wrap-content ${user.flag == '2' ? 'disable':''}">
                  ...
                  `<div class="dropdown align-self-start">
                    ...
                    <div class="dropdown-menu dropdown-menu-end">
                      <a class="dropdown-item" href="javascript:void(0);" id="${user._id}" onclick="singleMessageCopy(this)" style="display:${none_editBtn}">Copier <i
                          class="ri-file-copy-line float-end text-muted"></i></a>
                          <a class="dropdown-item" href="javascript:void(0);" onclick="singleForwordMessage(this)" id="${user._id}" type="single" data-bs-toggle="modal" data-bs-target="#forwardContact-Modal">Transférer<i class="ri-chat-forward-line float-end text-muted"></i></a>
                    </div>
                  </div>`:''}
                </div>
                <div class="conversation-name receiver_name">${receiverName}</div>
            ...
            <div class="user-chat-content">
              <div class="ctext-wrap">
                ...
                `<div class="dropdown align-self-start">
                  ...
                  <div class="dropdown-menu dropdown-menu-end">
                    <a class="dropdown-item" href="javascript:void(0)" id="${user._id}" onclick="singleMessageCopy(this)" style="display:${none_editBtn}">Copier <i class="ri-file-copy-line float-end text-muted"></i></a>
                    <a class="dropdown-item" href="javascript:void(0);" id="${user._id}" onclick="singleMessageUpdate(this)" style="display:${none_editBtn}">Modifier <i class="ri-save-line float-end text-muted"></i></a>
                    <a class="dropdown-item" href="javascript:void(0);" onclick="singleForwordMessage(this)" id="${user._id}" type="single" data-bs-toggle="modal" data-bs-target="#forwardContact-Modal">Transférer<i class="ri-chat-forward-line float-end text-muted"></i></a>
                    <a class="dropdown-item" href="javascript:void(0);" id="${user._id}" onclick="singleMessageDelete(this)">Supprimer <i
                        class="ri-delete-bin-line float-end text-muted"></i></a>
                  </div>
                </div>`:''}
              </div>
        ...
        <div class="ms-4 me-0">
          <ul class="list-inline mb-0 font-size-18">
            ...
            <li class="list-inline-item dropdown">
              ...
              <div class="dropdown-menu dropdown-menu-end">
                <a class="dropdown-item" href="javascript:void(0);" id="${user._id}" onclick="singleMessageDelete(this)">Supprimer</a>
              </div>
            </li>
          </ul>
        </div>
...
function singleMessageDelete(message) {
  ...
  Array.from(remove_msg).forEach((element, index) => {
    ...
    element.querySelector('.ctext-wrap-content').innerHTML = 'Message supprimé';
...
groupsForm.addEventListener("submit", (e) => {
  ...
  if (name == '') {
    toastr.error('Le nom du groupe est requis', 'Erreur');
  }
  else {
    var contactList = [], f = 0
    document
    ...
    if (f == 1) {
      ...
      toastr.error('Veuillez sélectionner des membres du groupe !', 'Erreur');
    }
  }
});
            ...
            <div class="user-chat-content">
              <div class="ctext-wrap">
                ...
                <div class="dropdown align-self-start">
                  ...
                  <div class="dropdown-menu dropdown-menu-end">
                      <a class="dropdown-item" href="javascript:void(0)" id="${id}" onclick="groupMessageCopy(this)" style="display:${none_editBtn}">Copier <i class="ri-file-copy-line float-end text-muted"></i></a>
                      <a class="dropdown-item" href="javascript:void(0);" id="${id}" onclick="singleForwordMessage(this)" type="group"  data-bs-toggle="modal" data-bs-target="#forwardContact-Modal">Transférer<i class="ri-chat-forward-line float-end text-muted"></i></a>
                  </div>
                </div>
              </div>
            ...
            <div class="user-chat-content">
              <div class="ctext-wrap">
                ...
                <div class="dropdown align-self-start">
                  ...
                  <div class="dropdown-menu dropdown-menu-end">
                      <a class="dropdown-item" href="javascript:void(0)" id="${id}" onclick="groupMessageCopy(this)">Copier <i
                      class="ri-file-copy-line float-end text-muted"></i></a>
                      <a class="dropdown-item" href="javascript:void(0);" id="${id}" onclick="multiMessageUpdate(this)">Modifier <i class="ri-save-line float-end text-muted"></i></a>
                      <a class="dropdown-item" href="javascript:void(0);" id="${user}" onclick="singleForwordMessage(this)" data-bs-toggle="modal" data-bs-target="#forwardContact-Modal">Transférer<i class="ri-chat-forward-line float-end text-muted"></i></a>
                      <a class="dropdown-item" href="javascript:void(0);" id="${id}" onclick="groupMsgDelete(this)">Supprimer <i
                      class="ri-delete-bin-line float-end text-muted"></i></a>
                  </div>
                </div>
              </div>
      ...
      <div class="ms-4 me-0">
        <ul class="list-inline mb-0 font-size-18">
          ...
          <li class="list-inline-item dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-end">
              <a class="dropdown-item" href="javascript:void(0);" id="${id}" onclick="groupMsgDelete(this)">Supprimer</a>
            </div>
          </li>
        </ul>
      </div>
...
socket.on("groupInfo", ({ group }) => {
  ...
  const receiverData = `
        <div class="float-end">
          <button type="button" class="btn btn-light btn-sm" id="group_edit" onclick="edit_groupName(this)"><i class="ri-edit-fill me-1 ms-0 align-middle"></i> Modifier</button>
        </div>
        <div>
          <p class="text-muted mb-1">Nom</p>
  ...
  if (document.getElementById("g_chat_" + groupId) != undefined) {
    ...
    groupUsers.forEach((contact) => {
      if (contact.is_admin == 1 && contact.user_id[0] == userId) {
        ...
        setTimeout(function () {
          ...
          let exit_btn = `<a class="dropdown-item text-danger" onclick="deleteGroup('${contact.group_id}')" href="javascript:void(0);">Supprimer le groupe<i class="ri-delete-bin-line float-end text-danger"></i></a>`
    ...
    if (admin_c == 0) {
      ...
      setTimeout(function () {
        ...
        let exit_btn = `<a class="dropdown-item text-danger" id="${userId}" onclick="deleteGroupMember(this.id)" href="javascript:void(0);">Quitter le groupe<i class="ri-delete-bin-line float-end text-danger"></i></a>`
            ...
            <div class="user-chat-content">
              <div class="ctext-wrap">
                ...
                <div class="dropdown align-self-start">
                  ...
                  <div class="dropdown-menu dropdown-menu-end">
                    <a class="dropdown-item" href="javascript:void(0)" id="${group._id}" onclick="groupMessageCopy(this)" style="display:${none_editBtn}">Copier <i class="ri-file-copy-line float-end text-muted"></i></a>
                    <a class="dropdown-item" href="javascript:void(0);" id="${group._id}" onclick="singleForwordMessage(this)" type="group" data-bs-toggle="modal" data-bs-target="#forwardContact-Modal">Transférer<i class="ri-chat-forward-line float-end text-muted"></i></a>
                  </div>
                </div>
              </div>
            ...
            <div class="user-chat-content">
              <div class="ctext-wrap">
                ...
                <div class="dropdown align-self-start">
                  ...
                  <div class="dropdown-menu dropdown-menu-end">
                    <a class="dropdown-item" href="javascript:void(0)" id="${group._id}" onclick="groupMessageCopy(this)" style="display:${none_editBtn}">Copier <i class="ri-file-copy-line float-end text-muted"></i></a>
                        <a class="dropdown-item" href="javascript:void(0);" id="${group._id}" onclick="multiMessageUpdate(this)" style="display:${none_editBtn}">Modifier <i class="ri-save-line float-end text-muted"></i></a>
                        <a class="dropdown-item" href="javascript:void(0);" id="${group._id}" onclick="singleForwordMessage(this)" type="group" data-bs-toggle="modal" data-bs-target="#forwardContact-Modal">Transférer<i class="ri-chat-forward-line float-end text-muted"></i></a>
                    <a class="dropdown-item" href="javascript:void(0);" id="${group._id}" onclick="groupMsgDelete(this)">Supprimer <i
                        class="ri-delete-bin-line float-end text-muted"></i></a>
                  </div>
                </div>
              </div>
          ...
          <div class="ms-4 me-0">
            <ul class="list-inline mb-0 font-size-18">
              ...
              <li class="list-inline-item dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-end">
                  <a class="dropdown-item" href="javascript:void(0);" id="${group._id}" onclick="groupMsgDelete(this)">Supprimer</a>
                </div>
              </li>
            </ul>
          </div>
...
socket.on("currentUser", function ({ userInfo }) {
  console.log("Données utilisateur reçues:", userInfo); // Debug
  ...
  const profileDescription = document.getElementById('profile_description');
  if (profileDescription && userInfo.description) {
    profileDescription.textContent = userInfo.description;
    
    // Mettre à jour localStorage avec la description récupérée du serveur
    if (record && record.data && record.data.user) {
      record.data.user.description = userInfo.description;
      localStorage.setItem('user', JSON.stringify(record));
      console.log("Description mise à jour dans localStorage depuis currentUser:", userInfo.description);
    }
  }
  ...
  if (statusDisplay) {
    ...
    // Mettre à jour localStorage avec le statut récupéré du serveur
    if (record && record.data && record.data.user) {
      record.data.user.status = status;
      localStorage.setItem('user', JSON.stringify(record));
      console.log("Statut mis à jour dans localStorage depuis currentUser:", status);
    }
  }
  ...
  if (userInfo._id == userId) {
    ...
    console.log("Mise à jour des switches - Notification:", user_Notification, "Muted:", userMute_Notification); // Debug
    
    if (notificationSwitch) {
      notificationSwitch.checked = user_Notification == 1;
      console.log("Switch notification défini sur:", notificationSwitch.checked); // Debug
    }
    if (mutedSwitch) {
      mutedSwitch.checked = userMute_Notification == 1;
      console.log("Switch muted défini sur:", mutedSwitch.checked); // Debug
    }
  }
...
document.querySelector("#btnSave").addEventListener("click", (e) => {
  ...
  if (name == '') {
    toastr.error(`Le nom est requis`, "Erreur");
  }
  else {
    var userName = document.querySelectorAll(".user_name");
    Array.from(userName).forEach((element, index) => {
...
document.querySelector("#profile-img-file-input").addEventListener("change", function () {
  ...
  if (!imgList.includes(extName)) {
    toastr.error(`Image invalide`, "Erreur");
  } else {
    var reader = new FileReader();
    reader.addEventListener(
      "load",
...
function notification_switch(src) {
  ...
  console.log("Notification switch changed:", notification); // Debug
  ...
  if (record && record.data && record.data.user) {
    record.data.user.notification = notification;
...
function notification_muted_switch(src) {
  ...
  console.log("Notification muted switch changed:", is_muted); // Debug
  ...
  // Mettre à jour le localStorage
  if (record && record.data && record.data.user) {
    record.data.user.is_muted = is_muted;
    localStorage.setItem("currentUser", JSON.stringify(record));
  }
}

// Vérification des permissions média au chargement
function checkMediaPermissions() {
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    navigator.permissions.query({name: 'camera'}).then(function(result) {
      ...
      console.log('Impossible de vérifier les permissions caméra:', error);
    });
    
    navigator.permissions.query({name: 'microphone'}).then(function(result) {
      console.log('Permission microphone:', result.state);
    }).catch(function(error) {
      console.log('Impossible de vérifier les permissions microphone:', error);
    });
  } else {
    console.error('API getUserMedia non supportée par ce navigateur');
    toastr.warning('Les appels audio/vidéo ne sont pas supportés par votre navigateur', 'Attention');
  }
}

// Fonction pour tester la connectivité des appels
function testCallConnectivity() {
  console.log('Test de connectivité des appels:');
  console.log('- userId:', userId);
  console.log('- globalusername:', globalusername);
  console.log('- image:', image);
  
  const targetUserId = document.getElementById('vausername-input').value;
  console.log('- targetUserId:', targetUserId);
  
  const incomingButton = document.getElementById(`openincoming_${userId}`);
  console.log('- Bouton appel entrant trouvé:', !!incomingButton);
  
  const typeElement = document.getElementById('type');
  console.log('- Type d\'appel:', typeElement ? typeElement.innerHTML : 'non défini');
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setTimeout(() => {
    testCallConnectivity();
  }, 2000);
});

// Écouteurs pour les confirmations de mise à jour des notifications
socket.on("notificationUpdated", function({ success, notification, error }) {
  if (success) {
...
socket.on("notificationMutedUpdated", function({ success, is_muted, error }) {
  if (success) {
    console.log("Notification muted mise à jour avec succès:", is_muted);
  } else {
    console.error("Erreur lors de la mise à jour de notification muted:", error);
  }
});
...
socket.on("user-connected", (socket_id, socket_name) => {
  ...
  if (document.getElementById('s_chat_' + socket_id) != null) {
    ...
    document.querySelector('.receiver_status').innerHTML = `<i class="ri-record-circle-fill font-size-10 text-success d-inline-block ms-1"></i> Connecté(e)`;
  }
});
...
socket.on("user-disconnected", (socket_id) => {
  ...
  document.querySelector('.receiver_status').innerHTML = `<i class="ri-record-circle-fill font-size-10 text-secondary d-inline-block ms-1"></i> Déconnecté(e)`;
});
...
document.getElementById('callaudiomodel').addEventListener('click', () => {
  console.log('Bouton appel audio cliqué'); // Debug
  
  const typeElement = document.getElementById('type');
  const targetUserId = document.getElementById('vausername-input').value;
  
  if (!targetUserId) {
    toastr.error('Veuillez sélectionner un contact avant d\'appeler', 'Erreur');
    console.error('Aucun contact sélectionné pour l\'appel');
    return;
  }
  
  if (typeElement && typeElement.innerHTML == 'single') {
    console.log('Démarrage appel audio vers:', targetUserId);
    document.getElementById(`audiotext`).innerHTML = 'Appel en cours...';
    document.getElementById(`audiotext`).style.display = 'block';
    sendUsername('audio');
    startCall('audio');
  } else {
    console.error('Type d\'appel non défini ou incorrect');
    toastr.error('Erreur de configuration d\'appel', 'Erreur');
  }
})
document.getElementById('callvideomodel').addEventListener('click', () => {
  console.log('Bouton appel vidéo cliqué'); // Debug
  ...
  if (typeElement && typeElement.innerHTML == 'single') {
    console.log('Démarrage appel vidéo vers:', targetUserId);
    document.getElementById(`calltext`).innerHTML = "Appel en cours...";
...
document.getElementById('receivevcall').addEventListener('click', () => {
  console.log('Réception d\'appel acceptée'); // Debug
  callerring.pause();
  callerring.currentTime = 0;
  var rspid = document.getElementById(`icvid`).innerHTML;
  var callType = document.getElementById(`calltype`).innerHTML;
  
  console.log('Envoi answerd:', { rspid: rspid, callType: callType }); // Debug
  socket.emit('answerd', rspid, callType);
  
  if (callType == 'video') {
    document.getElementById("video-call-div").style.display = "block";
    console.log('Affichage video-call-div pour appel vidéo'); // Debug
  } else {
    document.getElementById("video-call-div").style.display = "none";
    console.log('Masquage video-call-div pour appel audio'); // Debug
  }
  
  document.getElementById(`opencallvideomodel`).click();
  document.getElementById(`cutincomingmodel`).click();
  
  console.log('Démarrage joinCall avec:', { callType: callType, userId: userId, rspid: rspid }); // Debug
  joinCall(callType, userId, rspid);
})
socket.on('itbusy', () => {
  document.getElementById(`calltext`).innerHTML = "La personne est occupée";
  document.getElementById(`audiotext`).innerHTML = "La personne est occupée";
...
socket.on("ringcalling", (uid, icid, name, image, ctype) => {
  console.log('Réception ringcalling:', {
    uid: uid,
    icid: icid,
    name: name,
    image: image,
  ...
  if (!isactive) {
    ...
    // Définir le type d'appel correctement
    if (ctype == 'audio') {
      document.getElementById(`calltype`).innerHTML = 'audio';
      document.getElementById(`callicon`).innerHTML = ' <i class="ri-phone-fill"></i>';
      document.getElementById(`calltypetext`).innerHTML = 'Appel audio entrant';
      console.log('Configuration pour appel audio'); // Debug
    } else {
      document.getElementById(`calltype`).innerHTML = 'video';
      document.getElementById(`callicon`).innerHTML = ' <i class="ri-vidicon-fill"></i>';
      document.getElementById(`calltypetext`).innerHTML = 'Appel vidéo entrant';
      console.log('Configuration pour appel vidéo'); // Debug
    }
    // Réduire le délai et améliorer la gestion
    setTimeout(function () {
      if (!cutingphone) {
        console.log('Démarrage de la sonnerie et notification pour:', ctype); // Debug
        callerring.play();
        requestNotificationPermissions();
        var instance = new Notification(
          name, {
          body: `Appel ${ctype} entrant`,
          icon: `assets/images/users/${image}`
        });
        
        // Essayer plusieurs méthodes pour ouvrir la modal
        console.log('Tentative d\'ouverture de la modal:', `openincoming_${userId}`); // Debug
        let incomingButton = document.getElementById(`openincoming_${userId}`);
        
        if (!incomingButton) {
          // Fallback: chercher par classe
          incomingButton = document.querySelector('.openincomingclass');
          console.log('Utilisation du fallback, bouton trouvé:', !!incomingButton); // Debug
        }
        
        if (incomingButton) {
          incomingButton.click();
          console.log('Modal d\'appel entrant ouverte pour type:', ctype); // Debug
        } else {
          console.error('Aucun bouton d\'appel entrant trouvé'); // Debug
          // Forcer l'ouverture de la modal directement
          const modal = document.getElementById('incomingvideo');
          if (modal) {
            const bootstrapModal = new bootstrap.Modal(modal);
            bootstrapModal.show();
            console.log('Modal ouverte directement'); // Debug
          }
        }
      }
    }, 1000); // Réduire le délai de 2500ms à 1000ms
  } else {
...
function startCall(ctype) {
  console.log('Démarrage de l\'appel, type:', ctype); // Debug
  isactive = true;
  audioring.play();
  ...
  console.log('Contraintes média:', { video: videoConstraints, audio: true }); // Debug
  ...
  navigator.mediaDevices.getUserMedia({
    ...
    console.error('Erreur d\'accès aux médias dans startCall:', error);
    
    // Gestion spécifique des erreurs
    if (error.name === 'NotReadableError') {
      toastr.error('La caméra est peut-être utilisée par une autre application. Veuillez fermer les autres applications utilisant la caméra et réessayer.', 'Erreur caméra');
    } else if (error.name === 'NotAllowedError') {
      toastr.error('Accès à la caméra/microphone refusé. Veuillez autoriser l\'accès dans les paramètres du navigateur.', 'Permission refusée');
    } else if (error.name === 'NotFoundError') {
      toastr.error('Aucune caméra ou microphone trouvé sur cet appareil.', 'Périphérique non trouvé');
    } else {
      toastr.error('Impossible d\'accéder à la caméra/microphone: ' + error.message, 'Erreur');
    }
...
function joinCall(jctype, userId, rspid) {
  console.log('Rejoindre l\'appel, type:', jctype, 'de:', userId); // Debug
  isactive = true;
  ...
  navigator.mediaDevices.getUserMedia({
    ...
    console.error('Erreur d\'accès aux médias lors de joinCall:', error);
    ...
    // Essayer un appel audio seulement en fallback
    if (jctype === 'video') {
      ...
      navigator.mediaDevices.getUserMedia({
        ...
        toastr.info('Appel converti en mode audio seulement', 'Information');
      }).catch((audioError) => {
        console.error('Erreur fallback audio:', audioError);
        toastr.error('Impossible d\'accéder au microphone', 'Erreur');
        document.getElementById(`cutincomingmodel`).click();
        cutphones();
      });
    } else {
...
function muteAudio() {
  console.log('muteAudio appelé, localStream:', localStream, 'isAudio:', isAudio); // Debug
  
  if (!localStream) {
    console.error('localStream non défini dans muteAudio');
    toastr.error('Aucun stream audio actif', 'Erreur');
    return;
  }
  ...
  if (audioTracks.length === 0) {
    ...
    toastr.warning('Aucun microphone actif pour cet appel', 'Information');
    return;
  }
...
function muteVideo() {
  console.log('muteVideo appelé, localStream:', localStream, 'isVideo:', isVideo); // Debug
  
  if (!localStream) {
    console.error('localStream non défini dans muteVideo');
    toastr.error('Aucun stream vidéo actif', 'Erreur');
    return;
  }
  ...
  if (videoTracks.length === 0) {
    ...
    toastr.warning('Aucune caméra active pour cet appel', 'Information');
    return;
  }
...
// Gestion de l'édition de la description du profil
document.addEventListener('DOMContentLoaded', function() {
  const descriptionEdit = document.getElementById('description_edit');
  const editDescriptionDiv = document.getElementById('edit-description');
  const profileDescription = document.getElementById('profile_description');
  const descriptionTextarea = document.getElementById('description_textarea');
  const charCount = document.getElementById('char_count');
  const cancelDescription = document.getElementById('cancel_description');
  ...
  if (descriptionEdit) {
    descriptionEdit.addEventListener('click', function() {
      if (profileDescription && descriptionTextarea && editDescriptionDiv) {
        // Copier le texte actuel dans le textarea
        descriptionTextarea.value = profileDescription.textContent.trim();
        updateCharCount();
        
        // Basculer l'affichage
        profileDescription.classList.add('visually-hidden');
        descriptionEdit.classList.add('visually-hidden');
        editDescriptionDiv.classList.remove('visually-hidden');
  ...
  if (cancelDescription) {
    cancelDescription.addEventListener('click', function() {
      if (profileDescription && editDescriptionDiv && descriptionEdit) {
        // Restaurer l'affichage
        editDescriptionDiv.classList.add('visually-hidden');
        profileDescription.classList.remove('visually-hidden');
        descriptionEdit.classList.remove('visually-hidden');
  ...
  if (saveDescription) {
    saveDescription.addEventListener('click', function() {
      if (descriptionTextarea && profileDescription) {
        ...
        // Émettre l'événement socket pour sauvegarder
        socket.emit("updateUserDescription", { 
          userId: userId, 
  ...
  // Gérer la touche Entrée (Ctrl+Entrée pour sauvegarder)
  if (descriptionTextarea) {
...
// Écouter la réponse du serveur pour la mise à jour de la description
socket.on("userDescriptionUpdated", function({ userInfo }) {
  console.log("Description utilisateur mise à jour:", userInfo);
  ...
  // Mettre à jour localStorage pour persister la description
  if (record && record.data && record.data.user) {
    record.data.user.description = userInfo.description;
    localStorage.setItem('user', JSON.stringify(record));
    console.log("Description sauvegardée dans localStorage:", userInfo.description);
  }
});
...
function updateUserStatus(status) {
  ...
  if (!userId) {
    ...
    toastr.error("Erreur: utilisateur non identifié", "Erreur");
    return;
  }
  ...
  // Envoyer au serveur
  socket.emit("updateUserStatus", { userId: userId, status: status });
}

// Écouter la réponse du serveur pour la mise à jour du statut
socket.on("userStatusUpdated", function({ userInfo, error }) {
  if (error) {
    console.error("Erreur lors de la mise à jour du statut:", error);
    toastr.error("Erreur lors de la mise à jour du statut: " + error, "Erreur");
    return;
  }
  
  console.log("Statut utilisateur mis à jour:", userInfo.status);
  
  // Mettre à jour l'affichage
  const statusDisplay = document.getElementById('status-display');
  if (statusDisplay && userInfo.status) {
    statusDisplay.innerHTML = `${userInfo.status} <i class="mdi mdi-chevron-down"></i>`;
  }
  
  // Mettre à jour localStorage pour persister le statut
  if (record && record.data && record.data.user) {
    record.data.user.status = userInfo.status;
    localStorage.setItem('user', JSON.stringify(record));
    console.log("Statut sauvegardé dans localStorage:", userInfo.status);
  }
