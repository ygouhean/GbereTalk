...
// current user description edit
async function userDescriptionUpdate(id, description) {
  const user_update = await User.findByIdAndUpdate(id, { description }, { new: true });
  return user_update;
}

async function userStatusUpdate(id, status) {
  const user_update = await User.findByIdAndUpdate(id, { status }, { new: true });
  return user_update;
}
